...
impl VaultData {
    pub fn new(path: &str) -> error::Result<VaultData> {
        ...
        let mut file = fs::File::open(path).map_err(|e| std::io::Error::new(
            e.kind(),
        ...
        let iv = io::read_bytes(&mut file, AES_IV_LEN.value, Start(8))
            .map_err(HErr::Io)?.try_into().unwrap();
...
impl EncryptedFile {
    pub fn new(path: &str) -> error::Result<EncryptedFile> {
        ...
        Ok(EncryptedFile { path: path.into(), master_iv, iv, cypher_key, data_offset })
    }
}
...
mod util {
    ...
    pub fn generate_iv() -> AesIV {
        generate_random_data::<{ AES_IV_LEN.value }>().unsecure().try_into().unwrap()
    }

    pub fn generate_salt() -> Salt {
        generate_random_data::<{ HMAC_SALT_LEN.value }>().unsecure().try_into().unwrap()
    }
}
...
pub fn get_master_key(master_pw: &SecureString, vault_data: &VaultData) -> error::Result<AesKey> {
    ...
    derive(PBKDF2_ALGO.value, PBKDF2_ITERATIONS.value, &vault_data.salt, util::raw(master_pw), & mut key);
    match decrypt(Cipher::aes_256_cbc(), &key, Some(&vault_data.iv), &cypher_key) {
        Ok(key) => Ok(SecureArray::new(key.try_into().unwrap())),
        Err(msg) => {
            if msg.errors().first().is_some_and(|e| e.code() == OPEN_SSL_BAD_DECRYPT) {
...
pub fn decrypt_text_file(file: &EncryptedFile, master_key: &AesKey) -> error::Result<SecureVec<u8>> {
    type HErr = error::InvalidHeader;
    let key = decrypt(Cipher::aes_256_cbc(), master_key.unsecure(),
                      Some(&file.master_iv), &file.cypher_key).map(SecureVec::new)?;
...
pub fn generate_file_header(master_key: &AesKey) -> error::Result<(Vec<u8>, AesKey, AesIV)> {
    ...
    let mut encrypted_key = encrypt(Cipher::aes_256_cbc(), master_key.unsecure(),
...
pub fn encrypt_text_file(data: &SecureString, master_key: &AesKey) -> error::Result<Vec<u8>> {
    ...
    let mut encrypted_data = encrypt(Cipher::aes_256_cbc(), file_key.unsecure(),
...
pub fn crypto_write<Source, Dest>(source: Source, dest: &mut Dest, key: &AesKey, iv: &AesIV) -> error::Result<()>
where
...
pub fn crypto_read<Source, Dest>(source: Source, dest: &mut Dest, key: &AesKey, iv: &AesIV) -> error::Result<()>
where
    Source: Read,
    Dest: Write
{
